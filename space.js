...
function animate() {
  ...
  function gameOver () {
    ...
    // let messageGameOver = 'Game Over'
    // context.fillText(messageGameOver, canvas.width*0.5, canvas.height*0.5 - 20)
...
function fireProjectile() {
  ...
  if (now - lastFiredTime > FIRE_DELAY) {
    // verifique se ainda há espaço para outro projétil
    if (projectiles.length < MAX_PROJECTILES) {
      // crie um novo projétil
      projectiles.push(
        new Projectile({
          position: {
      ...
      // atualize o valor de lastFiredTime
      lastFiredTime = now;
    }
  }
}
...
buttonAttack.addEventListener('touchend', () => {
  keys.space.pressed = false;
})
