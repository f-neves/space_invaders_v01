...
class Grid {
  constructor() {
    ...
    this.invaders = [];
    this.width = 35 * (5 + Math.floor(Math.random() * 5));
    ...
    for (let i = 0; i < columns; i++) {
      for (let j = 0; j < rows; j++) {
        ...
        const invader = new Invader({ position });
...
for (let i = 0; i < 70; i++) {
  ...
  const velocity = { x: 0, y: Math.random() * 0.3 };
...
function updateInvaderProjectiles() {
  invaderProjectiles.forEach((invaderProjectile, index) => {
    ...
    if (hasProjectileHitPlayer(invaderProjectile)) {
      handleProjectileHitPlayer(invaderProjectile, index);
    }
  });
}

function hasProjectileHitPlayer(projectile) {
  return (
...
function handleProjectileHitPlayer(projectile, index) {
